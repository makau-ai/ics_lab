...
and a second "master" from a new IP is accepted alongside the real one. The twin's data-diode intent
(telemetry up, command/setpoint **down denied**) is enforced only by the `zone-fw` conduit rules in the
full twin, **not** by the broker in the default insecure lab.
