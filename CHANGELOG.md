# Revision 5 — A narrated, multilingual Guided Tour

Revision 5 adds a one-click **Guided Tour** of the whole platform. A "Take the guided tour" button on
the Learning Path opens a full-screen, self-narrating walkthrough of all eight sections — Orientation,
the four analysis levels, Detection, the Machine Problem, and the digital twin. It was built with a
persona workflow: a Senior Instructional Designer and an OT/ICS SME authored the narration, an SME
editor fact-checked it against the verified commands, nine native localization specialists translated
it, and four motion designers illustrated it.

**Narrated aloud, in your language, with closed captions.** Each step is spoken by the browser's Web
Speech API in a language chosen at runtime — English plus Spanish, French, German, Portuguese,
Mandarin, Japanese, Arabic (full right-to-left), Hindi, and Russian — with synchronized closed captions
that stay on when voice is unavailable. Controls cover play/pause, back/next, replay, speed, and voice,
plus an autoplay that narrates the entire path end to end.

**Real commands, real output, real roles.** Every demonstration shows the actual tshark / Zeek / twin
command and its real captured output — taken verbatim from the verified source, never regenerated —
next to the O*NET occupation that does this work in the field and a plain-language note on the
real-world stakes.

**A built scene for every step.** Each section has a self-contained animated SVG that ties the command
to the real OT process it maps to: the MQTT star and the intruder node, the exploded DNP3 packet with
its link-address-versus-IP contradiction, the forged-trip breaker, and the full five-zone wastewater
lift station with its wet-well, pumps, high-high float, and SSO spill gauge. All motion is disabled
under `prefers-reduced-motion`, and the tour is keyboard- and screen-reader-accessible.

Generated by `build/build_tour.py` from `build/tour_data.json`; entry point `curriculum/tour.html`.
